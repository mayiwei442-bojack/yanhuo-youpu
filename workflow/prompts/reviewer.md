...
For a source-image refresh, also fail when the hero image is missing, any public recipe step lacks an image, the image order/semantics do not match the mapped steps, the media recipe URL differs from the selected textual source, or text and media from different sources/variants are combined. A switch from source A to source B is valid only when the complete ingredients, method, hero, and all step images move to B together.
