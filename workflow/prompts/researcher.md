...
10. For a source-image refresh, verify the recipe's actual text source instead of trusting a stale default `source` field. Require one finished-dish image plus one semantically matching image for every public recipe step, all belonging to that same recipe page/source. Record the recipe page, any same-site media page, author/attribution, rights notice, original image URLs, HTTP status/content type, page order, adjacent step text, and the exact step mapping. If source A lacks a complete image set, source B may be selected only when the entire ingredient list, method, hero image, and every step image can move to B together. Never mix A text with B images or images from different recipes. If neither source qualifies, return FAIL and stop the batch without bypassing access controls.
