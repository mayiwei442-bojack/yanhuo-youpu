...
Modify only the target call in `tools/recipe_data.mjs`. Keep its name identity, English name, region, array position, and every unrelated recipe unchanged. In an ordinary recipe update, keep the image path and source-field shape unchanged. In an explicitly requested source-image refresh, add or replace media fields only from the Researcher mapping: the hero image and every step image must belong to the same primary recipe source as the complete ingredients and method. If source B is selected because A has no complete image set, rewrite the whole target recipe to B before attaching B media; never retain A text with B images. Do not edit generated `data/recipes.js`; the orchestrator regenerates it. Do not browse, query RAG, use Git, or call a standalone chat-model API.
