...
- Xiachufang intermittently redirects to a sliding CAPTCHA. The workflow records that failure and never attempts a bypass.
...
- A retry hit Xiachufang's CAPTCHA. The ingestion runner now reuses an existing Supabase document on idempotent reruns and only refetches when no cached document exists or `--refresh-web` is explicit.
- Douguo changed only a dynamic SEO summary between two test fetches of 锅包肉. The precisely identified older incomplete test document was removed; the retained seed result is exactly 10 entities, 20 documents, 100 chunks, 100 stored 1024-dimensional vectors, and 50 entity/chunk-type combinations.
