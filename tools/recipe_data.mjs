...
export const chinese = [
  ...
  c("清蒸鲈鱼", "Steamed Sea Bass", "粤菜", "整条鲈鱼1条，约680克；姜3汤匙，切细丝并分成两半；葱2根，切细丝并将葱白、葱绿分开；新鲜香菜8枝，粗略切碎；菜籽油1/4杯加2汤匙（分次使用）；酱汁用清水1/4杯；盐1/4茶匙；白砂糖3/4茶匙；生抽1/4杯；现磨白胡椒适量；蒸锅用清水适量", "1）检查整条鲈鱼，用带锯齿的牛排刀刮净残留鱼鳞，重点检查鱼腹、边缘、背鳍附近和鱼头；用厨房剪刀小心剪除坚硬鱼鳍，保留鱼头和鱼尾，操作时谨防鱼鳍扎手。2）检查鱼腹内靠近脊骨的位置，刺破并剪开覆盖血线的膜，用手指或勺子彻底清净血线；确认鱼鳃已经去除，如有残留，用厨房剪刀剪掉并冲洗干净。3）将鱼最后冲洗一遍，抖掉多余水分但不要擦干，放在耐热长盘中；蒸制前不要在鲜鱼上放盐、调味料或料酒。4）炒锅中加入适量蒸锅用清水，放入金属蒸架，将装鱼的耐热盘置于蒸架上，确保盘子高于水面；如需增加高度，可将蒸架垫在两端均已去除的金属罐上。5）盖好锅盖蒸9分钟，然后关火；用黄油刀检查鱼肉，鱼肉应直到贴骨处均已不透明，而鱼骨仍略呈半透明。6）小心将盘中积聚的液体彻底倒掉，在鱼身上铺一半姜丝、葱绿丝和8枝粗略切碎的新鲜香菜。7）将1/4杯清水、1/4茶匙盐、3/4茶匙白砂糖、1/4杯生抽和适量现磨白胡椒放入小碗或量杯，混合均匀。8）在小锅中放入额外的2汤匙菜籽油和剩余一半姜丝，加热至姜丝开始发出滋滋声；倒入调好的酱汁，继续加热至微沸。9）酱汁微沸后加入剩余1/4杯菜籽油和葱白丝，搅拌并加热至液体再次微沸并发出滋滋声；将全部酱汁均匀舀在鱼身上，趁热食用。", "09-qingzheng-luyu.png"),
